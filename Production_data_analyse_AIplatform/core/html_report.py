...
def generate_report(
    analysis_data: dict,
    output_path: str,
    title: str = "CPK Analysis Report - Zillnk",
...
def _station_summary(station_types: list, station_info: dict) -> str:
    ...
    n = len(station_types)
    if not station_info:
        return f'{n} station type(s)'
    parts = []
    for stype in station_types:
        cnt = station_info.get(stype)
        parts.append(f'{stype}&nbsp;×&nbsp;{cnt}' if cnt else stype)
    return f'{n} station type(s): ' + ', '.join(parts)
...
<div class="header">
  ...
  <div class="header-sub">Generated: {now} &nbsp;|&nbsp; {_station_summary(station_types, station_info)}</div>
</div>
...
<div id="range-modal" class="modal-overlay" style="display:none"
  ...
  <div class="modal-box" onclick="event.stopPropagation()">
    <div class="modal-header">
      <div class="modal-title">Range Search Results</div>
...
function drawChart(stationType, sheetName, pointName) {{
  ...
  if (!sigma || sigma === 0 || rawVals.length < 2) {{
    ...
    ctx.fillText('Insufficient data to plot distribution', W / 2, H / 2);
    // Update info label
  ...
  if (hasAnyFail) {{
    ...
    ctx.fillStyle = '#333';
    ctx.fillText('Pass', padL + 19, padT + 13);
    ctx.fillStyle = 'rgba(220,50,50,0.8)';
    ctx.fillRect(padL + 56, padT + 4, 12, 10);
    ctx.fillStyle = '#333';
    ctx.fillText('Fail', padL + 71, padT + 13);
  }}
...
function _updateChartInfo(stationType, sheetName, pt, pointName) {{
  ...
  const passSpan = nFail > 0
    ? `<span style="margin-right:16px;color:${{passColor}}"><b>Pass Rate:</b> ${{passRate}}%&nbsp;(${{nPass}} pass/${{nFail}} fail)</span>`
    : '';
  el.innerHTML = `
    <span style="margin-right:16px"><b>Test Item:</b> ${{pointName}}</span>
...
function doRangeSearch(stationType, sheetName) {{
  ...
  if (!pointName) {{ alert('Please select a test item'); return; }}
  if (isNaN(lo) || isNaN(hi)) {{ alert('Please enter a valid numeric range'); return; }}

  const shData = ALL_DATA[stationType] && ALL_DATA[stationType][sheetName];
  if (!shData || !shData[pointName]) {{ alert('No data found'); return; }}
  ...
  let html = isExact
    ? `<p>Exact match: <b>${{pointName}}</b> = <b>${{lo}}</b> &nbsp;|&nbsp; Hits: <b>${{matched.length}}</b></p>`
    : `<p>Range search: <b>${{pointName}}</b> &nbsp; [<b>${{lo}}</b> ~ <b>${{hi}}</b>] &nbsp;|&nbsp; Hits: <b>${{matched.length}}</b></p>`;
  if (matched.length === 0) {{
    html += '<p style="color:#888">No matching barcodes</p>';
  }} else {{
    html += '<table class="modal-table"><thead><tr><th>Barcode</th><th>Measured Value</th></tr></thead><tbody>';
...
<div class="sheet-panel" id="{panel_id}" data-station="{stype}" style="display:{display}">
  ...
  <div class="search-bar">
    ...
    <div class="search-group range-group">
      ...
      <input type="number" id="{range_lo_id}" placeholder="Min (equal = exact match)" step="any" style="width:160px"/>
      <span>~</span>
      <input type="number" id="{range_hi_id}" placeholder="Max" step="any" style="width:90px"/>
      <button class="btn btn-sm" onclick="doRangeSearch('{_esc_js(stype)}', '{_esc_js(sheet)}')">Find Barcodes</button>
    </div>
  </div>
    ...
    <table id="{table_id}" class="data-table">
      <thead>
        <tr>
          <th>Test Item</th>
          <th>N</th>
          <th class="pass-rate-col" style="display:none">Pass Rate</th>
          <th>Mean</th>
          <th>Std Dev</th>
          <th>Min</th>
          <th>Max</th>
...
{rows_html}
  ...
  <div class="chart-section">
    <div class="chart-tip">Click a row in the table above to view the distribution chart for that test item</div>
