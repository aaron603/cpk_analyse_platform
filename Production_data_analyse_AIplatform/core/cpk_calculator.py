...
# ---------------------------------------------------------------------------
# Log-based CPK fallback (for products with no xlsx/json test files)
# ---------------------------------------------------------------------------

_LOG_CRITICAL_RE = re.compile(
    r'CRITICAL\s+-\s+<string>\s+-\s+(.+?),\s*data=([\d.\-+eE]+)\((.+?)\),\s*'
    r'limit=\[([^\]]*)\],\s*result=(Pass|Fail)',
    re.IGNORECASE,
)


def analyze_log_folder(station_folder: str, log_cb=None) -> dict:
    """
    Fallback CPK analysis when no xlsx/json files are available.

    Walks *station_folder* recursively for ``ate_test_log.log`` files,
    parses every ``CRITICAL - <string>`` line (both Pass and Fail) to
    collect measurement values per test item, then computes CPK statistics.
    ...
    The barcode is taken from the grandparent directory of the log file.

    Returns the same format as ``analyze_xlsx_folder()``::

        {
          sheet_name: {
            point_name: {n, mean, std, min, max, lsl, usl,
                          cp, cpl, cpu, cpk,
                          values: [(barcode, value, is_pass), ...]}
          }
        }
    """
    def _log(msg):
    ...
    root = Path(station_folder)
    if not root.is_dir():
        return {}

    log_files = list(root.rglob('ate_test_log.log'))
    if not log_files:
        _log(f'  [INFO] 日志CPK: {station_folder} 中未找到 ate_test_log.log')
        return {}

    _log(f'  [INFO] 日志CPK分析: 找到 {len(log_files)} 个 ate_test_log.log 文件')

    # collected[item_name] = {values: [(bc, float, is_pass)], lsl, usl}
    collected: dict = {}

    for log_path in log_files:
        # Barcode is the grandparent dir: .../barcode/timestamp/ate_test_log.log
        try:
            barcode = log_path.parent.parent.name
        except Exception:
            barcode = 'unknown'
        ...
        try:
            ...
            _log(f'  [WARN] 读取日志失败 {log_path.name}: {exc}')
            continue

        for m in _LOG_CRITICAL_RE.finditer(text):
            item_name, raw_val, _unit, raw_limits, result = m.groups()
            item_name = item_name.strip()

            try:
                val = float(raw_val)
            except ValueError:
                continue  # Skip non-numeric values (firmware versions, etc.)

            # Parse limits: "[lsl, usl]" — may contain None/quoted strings
            lsl = usl = None
            try:
                parts = raw_limits.replace("'", '').split(',')
                if len(parts) >= 2:
                    s0, s1 = parts[0].strip(), parts[1].strip()
                    if s0 and s0.lower() not in ('none', 'null', ''):
                        lsl = float(s0)
                    if s1 and s1.lower() not in ('none', 'null', ''):
                        usl = float(s1)
            except Exception:
                pass

            is_pass = result.strip().lower() == 'pass'

            if item_name not in collected:
                collected[item_name] = {'values': [], 'lsl': None, 'usl': None}
            collected[item_name]['values'].append((barcode, val, is_pass))
            # Always overwrite limits — last file wins (consistent with xlsx mode)
            if lsl is not None:
                collected[item_name]['lsl'] = lsl
            if usl is not None:
                collected[item_name]['usl'] = usl

    if not collected:
        _log(f'  [WARN] 日志CPK: 未从日志中解析到任何 CRITICAL 测量行')
        return {}

    _log(f'  [INFO] 日志CPK: 解析到 {len(collected)} 个测试项')

    results: dict = {}
    skipped = 0

    for item_name, data in sorted(collected.items()):
        raw_vals = [v for _, v, _ in data['values']]

        if len(raw_vals) < 2 or len(set(raw_vals)) < 2:
            skipped += 1
            continue

        lsl, usl = data['lsl'], data['usl']
        stats = calculate_cpk(raw_vals, lsl, usl)
        stats['values'] = data['values']
        n_pass = sum(1 for _, _, p in data['values'] if p)
        stats['n_pass'] = n_pass
        stats['n_fail'] = len(data['values']) - n_pass

        # Use item_name as both sheet and point (one item per logical group)
        sheet = item_name
        point = item_name
        if sheet not in results:
            results[sheet] = {}
        results[sheet][point] = stats

    _log(f'  [INFO] 日志CPK分析完成: {len(results)} 个有效测试项'
         f'（跳过固定值/样本不足 {skipped} 个）')
    return results