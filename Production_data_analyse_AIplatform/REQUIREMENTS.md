...
- 控制遍历测试数据时是否包含 debug 版本目录，默认 **否**，配置持久化
- **debug 目录判定**（同时满足才认定）：
  1. 目录名含 `Rx`（不区分大小写），位于产品版本同级，如 `Rx3`、`RxB`
  2. 该目录下测试记录的 HTML 文件中包含 `Debug Mode: True`
- debug 目录层级：`TestResult/{产品类别}/{工站类别}/Rx*/`（与正常版本目录平级）
- `debug`、`file_bk`、`env_comp` 等辅助子目录无论本设置如何始终跳过
- **程序自动检测文件类型**：根据目标文件夹下的实际文件决定读取 xlsx 还是 json，无需手动选择
...
| 菜单 | 子项 | 功能 |
|------|------|------|
| 故障关系库维护 | 导出故障关系库 | 将 fault_rules 表全量导出为 YAML 文件（可查看、备份、迁移） |
| 故障关系库维护 | 导入故障关系库 | 导入 YAML 格式的故障规则文件到 fault_database.db（增量合并：相同 keywords 更新，新增追加） |
| 帮助 | 使用帮助 | 弹出使用说明文本窗口 |
| 帮助 | 关于 | 版本信息 |
  ...
  TestResult/                        ← 固定目录名
    {product_category}/              ← 产品类别（ORBI_B3、ORBI_B40等）
      {station_type}/                ← 工站类型（FT1、FT2、Aging、VSWR等）
        {product_code}/              ← 正常版本目录（X11_X11、R1B等）
          {barcode}/                 ← 单条码 或 双条码(BC1_BC2)
            {YYYYMMDDHHMMSS[_sfx]}/  ← 单次测试记录（时间戳目录）
              ...
              *_MEASUREMENT_Zillnk.json  ← B3B40/PA板，含首次失败描述
              ...
              screen_*.png
              DutInfo_Mes=*.json
        Rx3/ RxB/ ...                ← debug版本目录（与product_code平级，默认跳过）
          {barcode}/                 ← 结构同正常版本
            {YYYYMMDDHHMMSS[_sfx]}/
              ate_test_log.html      ← 含 "Debug Mode: True"（确认依据）
              ...
```
...
| 辅助子目录 `debug`、`file_bk`、`env_comp` 等 | 始终跳过，不计入分析 |
| 版本级debug目录（名含`Rx`，HTML含`Debug Mode: True`） | 默认跳过；"包含debug数据"选**是**时纳入 |
...
{out_dir}/
  {产品类别}_{YYYYMMDD_HHMMSS}/              ← 本次运行专属子目录
    ...
    comprehensive_report.html                 综合分析报告（6 Tab：Overview/Fail Analysis/CPK Analysis/Distribution/Fail Patterns/Fault Replay；英文界面；所有模式）
...
由 `core/html_comprehensive_report.py` → `generate_comprehensive_report()` 生成，**全英文界面**（面向英文母语客户）。
...
**6 个 Tab 内容**（英文界面，报告面向英文母语客户）：

| Tab（显示名） | 内容 |
|-----|------|
| Overview | KPI卡片（总样本量/整体良率/失败条码数/CPK达标率）、良率趋势折线、失败类型饼图、测试大类汇总表 |
| Fail Analysis | Top 25 高频失败测试项柱图（点击可查看明细）、失败记录明细表（含SN搜索） |
| CPK Analysis | Cpk 横向柱图（色标分级 <1.0红/1.0-1.33橙/≥1.33绿）、完整 CPK 统计表（含搜索） |
| Distribution | 按测试项切换的堆叠直方图（pass蓝/fail红）、统计面板（n/均值/σ/LSL/USL）；首次点击时加载原始值 |
| Fail Patterns | 失败类型统计卡片、按小时热图（时序分布）、多失败项SN分析 |
| Fault Replay | 左侧SN列表（可搜索/过滤状态）、右侧逐Sheet展开测试结果；首次点击时加载明细数据 |
...
由 `core/html_report.py` → `generate_report()` 生成，自包含无外部依赖，**全英文界面**。

按工站类型 + 测试大项分组展示 CPK 统计表和分布图。

### 6.3 失败分析报告（`fail_analysis_report.html`）

由 `core/html_fail_report.py` → `generate_fail_report()` 生成，仅在 `folder_direct` 模式且存在失败记录时生成，**全英文界面**。
...
- 自动检测提取目录下的文件格式（xlsx 或 json），无需手动指定
...
**导入入口**：菜单 `故障关系库维护 → 导入故障关系库`  
**导出入口**：菜单 `故障关系库维护 → 导出故障关系库`

**推荐工作流**：
1. 运行 `fail_only` 或 `all_with_fail` 分析，得到 `rule_suggestions_*.yaml`
2. 用文本编辑器打开，填写未分类条目的 `fault_type` 和 `suggestion`
3. 菜单 `故障关系库维护 → 导入故障关系库` → 系统自动合并（相同 keywords 更新，新增追加）
4. 下次分析立即生效，未分类比例持续下降

**导出用途**：将现有规则库导出为 YAML 文件，用于备份、跨机器迁移或人工审查；导出文件可直接重新导入。
