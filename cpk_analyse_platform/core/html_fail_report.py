...
<!-- Pareto chart -->
<section>
  <h2>高频失败测试项 Pareto 分析（Top {len(pareto_items)}）</h2>
  {pareto_html}
</section>

<!-- Failed barcodes table -->
<section>
  <h2>失败条码列表 <span class="badge-fail">{total_fail_bc} 个</span></h2>
  {fail_bc_html}
</section>

<!-- Never-passed barcodes -->
<section>
  <h2>从未成功条码 <span class="badge-never">{total_never_bc} 个</span></h2>
  {never_html}
</section>
...
def _render_pareto(items: list) -> str:
    if not items:
        return '<p class="no-data">无失败测试项数据</p>'
    ...
    svg = (
        ...
        f'{"".join(bars_svg)}'
        f'{line_svg}{dots_svg}'
        f'<text x="{pad_l - 8}" y="{pad_t - 6}" font-size="9" fill="#1a237e">次数</text>'
        f'<text x="{w - pad_r + 6}" y="{pad_t - 6}" font-size="9" fill="#e53935">累计%</text>'
        f'</svg></div>'
    )
    return svg
...
def _render_fail_barcodes(rows: list) -> str:
    if not rows:
        return '<p class="no-data">无失败条码</p>'
    ...
    return (
        '<table>'
        '<thead><tr>'
        '<th style="text-align:left">条码</th>'
        '<th>工站</th><th>失败次数</th><th>通过次数</th>'
        '<th>失败测试项数</th><th>最近测试时间</th>'
        '</tr></thead>'
        f'<tbody>{trs}</tbody>'
        '</table>'
    )
...
def _render_never_barcodes(rows: list) -> str:
    if not rows:
        return '<p class="no-data">无从未通过条码</p>'
    ...
    return (
        ...
        '<thead><tr>'
        '<th style="text-align:left">条码</th>'
        '<th>工站</th><th>总测试次数</th><th>最近测试时间</th>'
        '</tr></thead>'
        f'<tbody>{trs}</tbody>'
        '</table>'
    )
