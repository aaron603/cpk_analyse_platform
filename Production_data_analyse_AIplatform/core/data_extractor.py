...
# ---------------------------------------------------------------------------
# Debug directory detection
# ---------------------------------------------------------------------------

_DEBUG_NAME_RE = re.compile(r'Rx', re.IGNORECASE)


def _looks_like_debug_dir(dirname: str) -> bool:
    """Fast name check: True if dirname contains 'Rx' (case-insensitive)."""
    return bool(_DEBUG_NAME_RE.search(dirname))


def _html_has_debug_true(html_path: str) -> bool:
    """Return True if the HTML file contains a 'Debug Mode: True' line."""
    try:
        with open(html_path, encoding='utf-8', errors='ignore') as _f:
            for _line in _f:
                if 'Debug Mode: True' in _line:
                    return True
                if 'Debug Mode:' in _line:
                    return False   # Line found but not True; stop scanning
    except OSError:
        pass
    return False


def _confirm_debug_via_html(dirpath: str) -> bool:
    """
    Scan up to 2 levels below `dirpath` for an HTML file that contains
    'Debug Mode: True'.  Returns True if confirmed, False otherwise.

    Structure probed:
      dirpath/                     ← debug version dir (e.g. Rx3/)
        <barcode-or-ts-dir>/       ← level 1
          <ts-dir>/*.html          ← level 2 (timestamp folder)
        <ts-dir>/*.html            ← level 1 (timestamp folder, shallower layout)
    """
    try:
        level1 = [d for d in os.listdir(dirpath)
                  if os.path.isdir(os.path.join(dirpath, d))][:6]
    except OSError:
        return False

    for d1 in level1:
        d1_path = os.path.join(dirpath, d1)
        # Level 1 may itself be a timestamp folder (shallower layout)
        try:
            for fname in os.listdir(d1_path):
                if fname.lower().endswith('.html'):
                    if _html_has_debug_true(os.path.join(d1_path, fname)):
                        return True
        except OSError:
            pass
        # Also check one level further (barcode → timestamp → html)
        try:
            level2 = [d for d in os.listdir(d1_path)
                      if os.path.isdir(os.path.join(d1_path, d))
                      and _is_timestamp_folder(d)][:3]
        except OSError:
            continue
        for d2 in level2:
            try:
                for fname in os.listdir(os.path.join(d1_path, d2)):
                    if fname.lower().endswith('.html'):
                        if _html_has_debug_true(
                                os.path.join(d1_path, d2, fname)):
                            return True
            except OSError:
                pass
...
def _is_debug_version_dir(dirpath: str) -> bool:
    """
    Return True if `dirpath` is a debug version directory:
      1. (fast) Directory name contains 'Rx' (case-insensitive).
      2. (confirm) An HTML file inside a timestamp sub-directory contains
         'Debug Mode: True'.
    Both conditions must be met.
    """
    if not _looks_like_debug_dir(os.path.basename(dirpath)):
        return False
    return _confirm_debug_via_html(dirpath)
...
def find_test_records(barcode: str, station_root: str,
                      include_debug: bool = False) -> list:
    """
    ...
    for root, dirs, files in os.walk(norm_root):
        ...
        if not include_debug:
            dirs[:] = [d for d in dirs
                       if not _is_debug_version_dir(
                           os.path.join(norm_root_r, d))]
...
def discover_barcodes(station_folders: list,
                      include_debug: bool = False) -> list:
    """
    ...
    for root_folder in station_folders:
        ...
        for dirpath, dirs, _files in os.walk(root_folder):
            ...
            if not include_debug:
                dirs[:] = [d for d in dirs
                           if not _is_debug_version_dir(os.path.join(dirpath, d))]
...
def _walk_all_pass_in_folder(station_root: str,
                             include_debug: bool = False) -> list:
    """
    ...
    for root, dirs, _files in os.walk(norm_root):
        ...
        if not include_debug:
            dirs[:] = [d for d in dirs
                       if not _is_debug_version_dir(os.path.join(norm_r, d))]
...
def _walk_all_records_in_folder(station_root: str, log_cb=None,
                                include_debug: bool = False) -> list:
    """
